...
{
    public class DrugPanelsMsSql
    {
        private int _acc_id { get; set; }
        public DrugPanelsMsSql(int acc_id)
        {
            _acc_id = acc_id;
        }
        ...
        public DrugTableData Get()
        {
            ...
            var proc = new Proc(SQL_ALL_DRUGS_PRESCRIBED, "labdaq_mssql") { { "acc_id", _acc_id } }.All();
            var all_drugs = proc.Select(s => new DrugConsistent
            {
            ...
            proc = new Proc(SQL_INCONSISTENT_TABLE2, "labdaq_mssql") { { "acc_id", _acc_id } }.All();
            var inconsistent2 = proc.Select(s => new DrugInconsistent2
            {
                TestId = s["TEST_ID"].ToString(),
        ...
        const string SQL_INCONSISTENT_TABLE2 = @"
            SELECT tst.test_id,
                ...
                cast(case when len(coalesce(tse.RESULT_TRANSLATION, '')) > 0 
                  and ( coalesce(tse.LESS_THAN, 0) > rs.RESULT_NUMERIC or coalesce(tse.GREATER_THAN, 0) < rs.RESULT_NUMERIC )
                then
                  tse.RESULT_TRANSLATION
                else
                  cast(rs.RESULT_NUMERIC as varchar)
                end as varchar) as measured_result_numeric,
                round(rs.HIGH_OR_SD, 0) as cutoff,
                'POSITIVE' as outcome,
            ...
            LEFT OUTER JOIN test_events tse on tse.test_id = tst.test_id and len( coalesce(tse.RESULT_TRANSLATION, '')) > 0 
